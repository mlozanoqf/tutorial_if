...
The payoff functions for European options are the following.

* Payoff from a long position in a European call option: $\mathrm{max}(S_T-K, 0)$. Or equivalently: $-\mathrm{min}(K-S_T, 0)$.
* Payoff from to the holder of a long position in a European put option: $\mathrm{max}(K-S_T, 0)$. Or equivalently: $-\mathrm{min}(S_T-K, 0)$.
...
```{r fig.cap="Hull: Figure 12.1a, profit diagram."}
S0 = 65
...
```{r fig.cap="Hull: Figure 12.1a, profit diagram (zoom)."}
S0 = 65
c_price = 5
# Here I assume the stock price is currently 65.
Sl <- ST.seq - S0
...
We can identify at least three isosceles right triangles, they have two sides of equal measure. Now see how it looks if we plot a payoff diagram instead.
