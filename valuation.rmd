...
Can we combine two losing investments into a winner?
...
```{r fig.cap="30-year cumulative return asset A, many paths, a density plot."}
a.30x10k |>
  filter(year == 30) |>
  ggplot(aes(log(c.ret))) +
  geom_density(size = 2) +
  theme(legend.position = "none", legend.title = element_blank()) +
  labs(x = "Log cumulative returns in year 30",
       subtitle = "10,000 paths of the evolution of $1 invested in year 0.")
```
...
```{r fig.cap="30-year Log10 cumulative returns asset A, B and portfolio C, many paths."}
set.seed(2, sample.kind = "Rounding")
...
abcx |>
  ...
  labs(y = "Log cumulative returns", x = "")
```
