...
```{r fig.cap="Call price as a function of $S_0$."}
# See the results.
...
```{r fig.cap="Put price as a function of $S_0$."}
...
```{r fig.cap="Call and put prices as a function of $S_0$ (zoom)."}
...
```{r fig.cap="Option values as a function of $S_0$ including option bounds."}
# Put lower bound.
...
```{r fig.cap="Call option changes as $S_0$ and $\\sigma$ increases: the role of bounds."}
...
```{r fig.cap="Put option changes as $S_0$ and $\\sigma$ increase: the role of bounds."}
# Evaluate the function.
...
legend("bottomleft", legend=expression(sigma == 0.3, sigma == 1.5,
       ...
       lty = c(1, 1, 1, 2), bg = "white", cex = 0.8)
```
```{r fig.cap="Put option changes as $S_0$ and $\\sigma$ increase: the role of bounds (zoom)."}
# Evaluate the function.
p.S0.s1 <- mapply(p.bs, S0.seq, K, rf, TT, sigma)
p.S0.s5 <- mapply(p.bs, S0.seq, K, rf, TT, sigma * 5)
p.S0.s15 <- mapply(p.bs, S0.seq, K, rf, TT, sigma * 15)
...
plot(S0.seq, p.S0.s5, type = "l", lwd = 3,  xlab = expression(paste(S[0])),
     ylab = "Pall option price, p", col = "blue",
...
```{r fig.cap="Call value as $S_0$ and $K$ change, a plane view."}
# K as a variable.
K.seq <- S0.seq
...
```{r fig.cap="Call value as $S_0$ and $K$ change, a contour view."}
...
```{r fig.cap="Call value as $S_0$ and $K$ change, an interactive view."}
...
```{r fig.cap="European put as $T$ increases: the mysterious hump."}
...
```{r fig.cap="European put value as $S_0$ and $T$ change: a plane."}
...
```{r fig.cap="Put value as $S_0$ and $T$ change, a contour view, or the hypnotic plot."}
...
```{r fig.cap="Put value as $S_0$ and $T$ change, an interactive view."}
...
```{r fig.cap="Portfolio A and C payoffs, $\\mathrm{max}(S_T,K)$."}
par(pty = "s")
...
```{r fig.cap="A synthetic stock $S_0$."}
par(pty = "s")
...
```{r fig.cap="A synthetic bond $Ke^{-rT}$."}
par(pty = "s")
...
```{r fig.cap="Binomial and Black-Scholes convergence, a panel view."}
par(mfrow = c(2, 2), mai = c(0.7, 0.4, 0.4, 0.4))
par(pty = "s")
plot(S0.seq, c.S0.s1, type = "l", ylim = c(0, 4), xlim = c(30, 45), 
     col = "black", lwd = 5, xlab = expression(paste(S[0])), 
     ylab = "European call")
lines(S0.seq, b2[1,], col = "purple", lwd = 2)
...
plot(S0.seq, c.S0.s1, type = "l", ylim = c(0, 4), xlim = c(30, 45), 
     ...
     ylab = "European call")
lines(S0.seq, b4[1,], col = "red", lwd = 2)
legend("topleft", legend = c("4-steps"),
       bg = "white", bty = "n")
par(pty = "s")
plot(S0.seq, c.S0.s1, type = "l", ylim = c(0, 4), xlim = c(30, 45), 
     col = "black", lwd = 5, xlab = expression(paste(S[0])), 
...
```{r fig.cap="$S_t$ binomial paths from $S_0$ to $S_T$."}
nona <- ! is.na(Spaths)
...
There is no difference between American and European call options theoretical prices as there are no incentives to exercise American call options early. The case of put options is different as incentives to exercise American put options early may arise. As a consequence, American put options are in general more expensive than European put options. Let's evaluate this difference in a 50-step case: $S_0 = 50$, $K=52$, $\sigma = 0.3$, $T=2$, $rf=0.05$.

```{r fig.cap="The value of being American; or the cost of being European."}
S.seq <- seq(0, 80, 0.5)
...
This section is based on @Hull section 13.1. The risk-neutral probability for the case of $rf=0.12$, $T=3/12$, $u=1.1$ and $d=0.9$:
...
Following @Hull, This option will have one of two values at the end of the 3 months. If the stock price turns out to be \$22, the value of the option will be \$1; if the stock price turns out to be \$18, the value of the option will be zero. This is the option expected value at time $T$.

$p\times(\$1) + (1-p)\times \$0 = 0.6522727.$
...
We can calculate the option value at $t=0$. This option value is valid for the real world, not only the risk-neutral world.

$0.6522727 \times e^{-rT} = 0.6329951.$

```{r}
option_0 <- option_T * exp(-0.12*(3/12))
option_0
```
Suppose that, in the real world, we know the expected return on the stock is 16%. This is easily calculated as we have historical information of stock returns. Then, the probability of an up movement in the real world $p^*$is:
...
What can we do to find out the option discount factor? Since we know the correct value of the option is $f=0.6329951$, we can deduce that the correct real-world discount rate.

$0.6329951 = 0.704053e^{-r\times3/12} \rightarrow \frac{0.6329951}{0.704053} = e^{-r\times3/12}$ 
...
The real-world discount rate for the option is 42.55688%. Let's put everything in a function.
...
```{r fig.cap="Options are in general much risker compared with the underlying stock."}
...
Consider asset A which has a known 6% gross return. In the context of a binomial tree: $[p\times(1+u)]+[(1-p)\times(1-d)]=1.06$. Let's assume $p=0.5$, so we have: $0.5(1+u)+0.5(1-d)=1.06$. Asset A has 40% volatility, so $0.4=\sqrt{0.5(1+u)^2+0.5(1-d)^2-1.06^2}$. We have two equations and two unknown variables, so we can solve for $(1+u)$ and $(1-d)$: $(1+u)=1.46$ and $(1-d)=0.66$.

In a period of 5 years, we would have a random path of ups (1.46) and downs (0.66) for asset A.
...
```{r fig.cap="5-year simulation of u and d for asset A."}
...
```{r fig.cap="5-year simulation of cumulative returns for asset A."}
plot(c(0:5), c(1, cumprod(a)), type = "b", lwd = 3,
     ylab = c("Cumulative $ return"), xlab = c("Time"))
```
...
```{r fig.cap="30-year cumulative return asset A."}
u = 0.46
d = 0.34
n = 30
set.seed(2, sample.kind = "Rounding")
...
ggplot(a.30, aes(x = year, y = ret)) +
  geom_line(size = 1) +
  geom_point(aes(x = 5, y = 0.6128265), size = 5) +
      labs(y = "Cumulative return ($)", x = "Year",
       subtitle = "Evolution of $1 invested in year 0.")
```

These results represent one single path. So, if we are interested in the most likely value of our investment we need to simulate many paths and then estimate the median. Let's simulate 10000 paths and see the most likely value of our $\$1$ dollar investment in 30 years.
...
```{r fig.cap="30-year cumulative return asset A, many paths."}
a.30x10k |>
...
```{r fig.cap="30-year Log10 cumulative return asset A, many paths."}
a.30x10k |>
ggplot(aes(x = year, y = c.ret, color = name)) +
  geom_line(size = 1) +
  geom_point(aes(x = 0, y = 1), size = 5) +
  theme(legend.position = "none", legend.title = element_blank()) +
  labs(y = "Log10 cumulative returns", x = "Year",
       subtitle = "10,000 paths of the evolution of $1 invested in year 0.") +
  scale_y_log10()
```
...
It would be a mistake to consider the mean as the maximum value is very high and not likely to happen. The asset A is then a loosing investment, $\$1$ would most likely lead to $\$0.5733923$ in 30 years.

Now, let's consider an asset B. Asset B has a known excess return of $-0.1\%$. In the context of a binomial tree: $0.5(1+u)+0.5(1-d)=0.999$. Asset B has no volatility, imagine is a Treasury Bill or a similar risk-free asset. Solving for $(1+u)$ and $(1-d)$ leads to $(1+u)=0.999$ and $(1-d)=0.999$. Asset B is for sure a loosing investment as well.
...
```{r fig.cap="30-year cumulative returns asset A, B and portfolio C."}
abc |>
  gather(A:C, key = name, value = c.ret) |>
  ggplot(aes(x = year, y = c.ret, color = name)) +
  geom_line(size = 1) +
  geom_point(aes(x = 0, y = 1), size = 5, col = "black") +
  labs(y = "Cumulative returns", x = "Year")
```

```{r fig.cap="30-year Log10 cumulative return asset A, B and portfolio C, many paths."}
set.seed(2, sample.kind = "Rounding")
