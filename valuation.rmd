...
```{r fig.cap="30-year cumulative returns asset A, B and portfolio C, one single path."}
abc |>
...
```{r fig.cap="30-year log cumulative returns asset A, B and portfolio C, only the final cumulative return."}
set.seed(2, sample.kind = "Rounding")
...
```{r fig.cap="30-year log cumulative returns asset A and portfolio C, only the final cumulative return, a density view."}
abcx |>
  gather(A:C, key = name, value = c.ret) |>
  filter(year == 30) |>
  filter(name == c("A", "C")) |>
