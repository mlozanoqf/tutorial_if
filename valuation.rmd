...
```{r fig.cap="Put option changes as $S_0$ and $\\sigma$ increases: the role of bounds."}
# Evaluate the function.
...
legend("bottomleft", legend=expression(sigma == 0.3, sigma == 1.5,
       ...
       lty = c(1, 1, 1, 2), bg = "white", cex = 0.8)
```
```{r fig.cap="Put option changes as $S_0$ and $\\sigma$ increases: the role of bounds (zoom)."}
# Evaluate the function.
p.S0.s1 <- mapply(p.bs, S0.seq, K, rf, TT, sigma)
p.S0.s5 <- mapply(p.bs, S0.seq, K, rf, TT, sigma * 5)
p.S0.s15 <- mapply(p.bs, S0.seq, K, rf, TT, sigma * 15)
...
```{r fig.cap="Put value as $T$ increases: the mysterious hump."}
...
```{r fig.cap="Put value as $S_0$ and $T$ change: a plane."}
...
In sum, this is the payoff of portfolios A and C: $\mathrm{max}(S_T,K)$.

```{r fig.cap="Portfolio A and C payoffs: $\\mathrm{max}(S_T,K)$."}
par(pty = "s")
plot(ST.seq, pc, type = "l", ylab = "Payoff", 
     xlab = expression(paste(S[T])), lwd = 3, ylim = c(0, 150))
```
...
plot(S0.seq, b1[1, ], type = "l", col = "red", lwd = 2, 
     xlab = expression(paste(S[0])), ylab = "Call")
...
plot(S.seq, unlist(AEoptions[4,]) - unlist(AEoptions[2,]), 
     type = "l", lwd = 2, main = "The value of being American.",
