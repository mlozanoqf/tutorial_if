...
```{r fig.cap="30-year cumulative return asset A, 1,586 selected paths."}
a.30x10k |>
  group_by(name) |>
  filter(c.ret < 1) |>
  filter(length(year) == 30) |>
...
```{r fig.cap="30-year log cumulative return asset A, 1,586 selected paths."}
a.30x10k |>
  group_by(name) |>
  filter(c.ret < 1) |>
  filter(length(year) == 30) |>
  ...
  subtitle = "Selected paths.")
```


Note the plot is highly concentrated at low values. It is easier to show the results in logarithm form.

```{r fig.cap="30-year Log cumulative return asset A, many paths."}
a.30x10k |>
ggplot(aes(x = year, y = log(c.ret), color = name)) +
  geom_line(size = 1) +
  geom_point(aes(x = 0, y = log(1)), size = 5, col = "black") +
  theme(legend.position = "none", legend.title = element_blank()) +
